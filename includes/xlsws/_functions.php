 ...
 */
    ...
    function _xls_get_conf($strKey, $mixDefault = "") {
        return Configuration::$Manager->GetValue(
            $strKey, $mixDefault);
    }
...
EOS;
    ...
    */
	...
	function _xls_array_search($needle, $haystack)
	{
		foreach($haystack as $elem){
			if(preg_match('/' . $elem.'/',$needle)){return true;}
		}
		return false;
	}	
    ...
    function _xls_add_meta_redirect($url , $delay = 60) {
        _xls_stack_add('xls_meta_redirect',
    ...
    */
    ...
    function _xls_tax_default_taxcode() {
        $taxcodes = TaxCode::LoadAll(QQ::Clause(QQ::OrderBy(
            QQN::TaxCode()->ListOrder)));
        ...
        $products = Product::QueryArray(
            QQ::AndCondition(QQ::Equal(QQN::Product()->Web, 1),
                QQ::OrCondition(
                    ...
                    QQ::AndCondition(QQ::Equal(QQN::Product()->MasterModel, 0),
